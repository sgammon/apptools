...
from util import AppToolsLogController
...
logger = AppToolsLogController('apptools.core')
...
class BaseHandler(RequestHandler, AssetsMixin):
    ...
    def baseHeaders(self):
        return {
            ...
            'X-Powered-By': 'Google App Engine/1.6.1',  # Indicate the SDK version
            'X-UA-Compatible': 'IE=edge,chrome=1',  # Enable compatibility with Chrome Frame, and force IE to render with the latest engine
            'Access-Control-Allow-Origin': '*'  # Enable Cross Origin Resource Sharing (CORS)
        }
    ...
    def render(self, path, context={}, elements={}, content_type='text/html', headers={}, **kwargs):
        ...
        try:
            ...
            if config.debug:
                raise e  # in production, the show must go on...
            else:
                pass
