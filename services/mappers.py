...
class JSONRPCMapper(service_handlers.JSONRPCMapper):
    ...
    def envelope(self, wrap, response):
        ...
        ## Compile signature
        signature = [
            svsconfig.get('secret_key', self._serviceConfig.get('secret_key', '__development__')),  # HMAC key
            str(response),  # message
            svsconfig.get('hmac_hash', hashlib.sha512)  # hash algorithm
        ]
        ...
        ## Add debug info
        if config.debug or self._serviceConfig.get('debug', False):
