    ...
    def __init__(cls, name, bases, dct):
        ...
        if name == 'PolyPro':
            ...
            if getattr(cls, '__class_hierarchy__', None):
                raise _apibridge.db.ConfigurationError(('%s cannot derive from PolyPro as '
            ...
            if cls.__class_hierarchy__[0] != cls.__root_class__:
                raise _apibridge.db.ConfigurationError(
...
class PolyPro(ldb.Model):
    ...
    def kind(cls):
        ...
        if cls.__name__ == 'PolyPro':
            return cls.__name__
        else:
            return cls.class_key()[0]
    ...
    def from_entity(cls, entity):
        ...
        if(_PATH_KEY_PROPERTY in entity and
           tuple(entity[_PATH_KEY_PROPERTY]) != cls.path_key()):
            ...
            try:
                ...
                raise _apibridge.db.KindError('Could not import model hierarchy \'%s\'' % str(key))
        ...
        if (_CLASS_KEY_PROPERTY in entity and
            ...
            try:
                ...
                raise _apibridge.db.KindError('No implementation for class \'%s\'' % (key,))
    ...
    def gql(cls, query_string, *args, **kwds):
        ...
        if cls == cls.__root_class__:
            ...
            query = _apibridge.db.GqlQuery('SELECT * FROM %s %s' % (cls.kind(), query_string))
